...
Alongside chatbots, the most frequently used generative AI applications are Text-to-Image models like DALL-E[^2], or Stable Diffusion[^3].
These are able to generate realistic and detailed images using textual instructions given by the user.

In this blog post, I intend to explain a common weakness of Text-to-Image generation models and explore one possible solution following the introduction of Uni-ControlNet[^1].
...
Let's say for example, we like this wallpaper of a Lamborghini driving on a racetrack and want to recreate a similar image with a different car.
We can start with a simple text prompt and let the model generate a first image.
Then we add more details to the prompt and try to influence the image generation to make the resulting image look more like our reference image.
We find that even with a very detailed description, it is difficult to directly influence the generated image.
In our example the reference image and the generated images show major differences.
...
References
======
[^1]: [Uni-ControlNet](https://arxiv.org/pdf/2305.16322)
[^2]: [DALL-E 3](https://openai.com/index/dall-e-3/)
[^3]: [Stable Diffusion 3](https://stability.ai/news/stable-diffusion-3)
[^4]: [ControlNet](https://arxiv.org/pdf/2302.05543)
[^5]: [GLIGEN](https://gligen.github.io/)
[^6]: [T2I-Adapter](https://arxiv.org/pdf/2302.08453)
